...
import { handelAuthRequest } from '@/components/utils/apiRequest';
...
const EditProfile = () => {
    ...
    const handleUpdateProfile = async () => {
        ...
        const result = await handelAuthRequest(updateProfileReq, setIsLoading);
    ...
    const handlePasswordChange = async (e:React.FormEvent) => {
        ...
        const result = await handelAuthRequest(updatePassReq,setIsLoading);
        if(result){
            dispatch(setAuthUser(result.data.data.user));
            toast.success(result.data.message);
        }
            ...
            <div className='w-[80%] mx-auto'>
                <div className='mt-16 pb-16 border-b-2 '>
                    ...
                    <div className='flex items-center justify-center'>
                        <LoadingButton isLoading={isLoading} size={"lg"} className='bg-blue-800 mt-4' onClick={handleUpdateProfile}>
                            Change Photo
                        </LoadingButton>
                    </div>
                </div>
                <div className='mt-10 border-b-2 pb-10'>
                    ...
                    <textarea value={bio} onChange={(e)=>setBio(e.target.value)} className='w-full h-[7rem] bg-gray-200 outline-none p-6 rounded-md'>
                    </textarea>
                    <LoadingButton isLoading={isLoading} size={"lg"} className=' mt-6' onClick={handleUpdateProfile}>
                            Change Bio
                        </LoadingButton>
                </div>
                <div>
                    ...
                    <form className='mt-8 mb-8' onSubmit={handlePasswordChange}>
                        ...
                        <div className='mt-6'>
                            <LoadingButton isLoading={isLoading}type='submit' className='bg-red-700'>
                                Change Password
                            </LoadingButton>
                        </div>
                    </form>
