    ...
    useEffect(()=>{
        if(!user){
            ...
            const getUser =async()=>{
                    ...
                    const result = await handelAuthRequest(getUserReq,setIsLoading);
                    if(result){
...
{/*             
    ...
    <Avatar className='w-[10rem] h-[10rem] mb-6 md:mb-0'>
        <AvatarImage src={userProfile?.profilePicture} className='h-full w-full rounded-full'/>
        <AvatarFallback>CN</AvatarFallback>
    </Avatar>
    ...
    <div className="flex flex-col space-y-4 ">
        ...
        {/* Bio */}
        <p className='w-[80%} font-medium'>{userProfile?.bio || "No bio available"}</p>
    </div>
</div>
<div className='mt-10'>
    <div className='flex items-center justify-center space-x-14'>
            <div className={cn("flex items-center space-x-2 cursor cursor-pointer",postOrSave==="POST" && "text-blue-500")} onClick={()=> setPostOrSave("POST")}>
                <Grid/>
                <span className='font-semibold'>Post</span>                
            </div>
            <div className={cn("flex items-center space-x-2 cursor cursor-pointer",postOrSave==="SAVE" && "text-blue-500")} onClick={()=> setPostOrSave("SAVE")}>
                <Bookmark/>
