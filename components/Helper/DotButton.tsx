...
const DotButton = ({post,user}:Props) => {
  ...
  const handleDeletePost = async()=>{
    ...
    const result = await handelAuthRequest(deletePostReq);
      ...
      <Dialog>
        ...
        <DialogContent>
          ...
          <div className='space-y-4 flex flex-col w-fit justify-center items-center mx-auto'>
            {!isOwnPost&&(
              <div>
                <Button onClick={()=>{if(post?.user?._id) handleFollowUnfollow(post?.user._id);}} variant={isFollowing?"destructive":"secondary" } className='text-gray-900'>
          ...
          {isOwnPost &&(
            <Button variant={"destructive"} onClick={handleDeletePost} className='text-gray-200'>
              Delete Post
            </Button>
          )}
          <DialogClose>
