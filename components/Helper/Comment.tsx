...
<Dialog>
    ...
    <DialogContent className='w-[95vw] max-w-none h-[50vh] p-0 flex gap-0 flex-col sm:flex-row'>
        <DialogTitle></DialogTitle>
        
       {/* Image Section - Left Half */}
<div className={`w-full sm:w-1/2 flex items-center justify-center bg-white rounded-l-lg h-full ${!post?.image?.url ? 'p-4' : ''}`}>
  {post?.image?.url ? (
    <Image
      src={`${post?.image?.url}`}
      alt="Post Image"
        ...
        <div className='w-full sm:w-1/2 flex flex-col justify-between h-full'>
            ...
            <div className='p-4'>
                <div className='flex items-center gap-2'>
                    ...
                    />
                    <Button variant={"outline"} onClick={()=>{if(post?._id)addCommentHandler(post._id)}}>Send</Button>
                </div>
            </div>
        </div>
    </DialogContent>
