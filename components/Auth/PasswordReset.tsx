...
const PasswordReset = () => {
    ...
    const handelSubmit = async()=>{
        ...
        const result = await handelAuthRequest(resetPassReq,setIsLoading);
        if(result){
  ...
  return (
    <div className='h-screen flex items-center justify-center flex-col'>
        ...
        <p className='mb-6 text-sm sm:text-base text-center text-gray-600 font-medium'>
            Enter yout OTP and new password to reset your password
        </p>
        ...
        <div className='mb-4 mt-4 w-[90%] sm:w-[80%] md:w-[60%] lg:w-[40%] xl:w-[30%] '>
            <PasswordInput name="password" placeholder='Enter new password' inputClassName='px-6 py-3 bg-gray-300 rounded-lg outline-none' value={password} onChange={(e)=>setPassword(e.target.value)}/>
        </div>
        <div className='w-[90%] sm:w-[80%] md:w-[60%] lg:w-[40%] xl:w-[30%] '>
            <PasswordInput name="passwordconfirm" placeholder='Confirm new password' inputClassName='px-6 py-3 bg-gray-300 rounded-lg outline-none' value={passwordConfirm} onChange={(e)=>setPasswordConfirm(e.target.value)}/>
        </div>
        <div className='flex items-center space-x-4 mt-6'>
            <LoadingButton onClick={handelSubmit} isLoading={isLoading}> Change Password</LoadingButton>
