...
const Verify = () => {
    ...
    const handleKeyDown = (index: number, event: KeyboardEvent<HTMLInputElement>):void => {
      ...
      const handleResendOtp =async()=>{
        ...
        const result = await handelAuthRequest(resendOtpReq,setIsLoading);
        if (result){
            toast.success(result.data.message);
        }
      };
  ...
  return (
    <div className='h-screen flex items-center flex-col justify-center'>
        ...
        <p className='mb-6 text-sm sm:text-bass text-gray-600 font-medium'>
            We have sent a code to {user?.email}
        </p>
        <div className='flex space-x-4'>
            {[0,1,2,3,4,5].map((index)=>{
                return (
                    <input type='number' key={index} maxLength={1}  className='sm:w-20 sm:h-20 w-10 h-10 rounded-lg bg-gray-200 text-lg sm:text-3xl font-bold outline-gray-500 text-center no-spinner' value={otp[index] || ""}  ref={(el) => { inputRefs.current[index] = el;}} onKeyDown={(e)=>handleKeyDown(index,e)} onChange={(e)=> handleChange(index,e)}/>
                 )
            })}
        </div>
